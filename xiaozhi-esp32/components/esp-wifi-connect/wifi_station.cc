...
void WifiStation::HandleScanResult() {
    ...
    if (connect_queue_.empty()) {
        ...
        // A hidden AP leaves the SSID blank in its beacons, so the name match
        // above can never find it. Queue every saved network for a blind
        // attempt: StartConnect() with channel 0 hands the SSID straight to
        // esp_wifi_connect(), which probes for it directly - and hidden APs do
        // answer a directed probe.
        if (!ssid_list.empty()) {
            ESP_LOGI(TAG, "No AP in scan, trying %d saved network(s) directly (hidden SSID?)",
                     static_cast<int>(ssid_list.size()));
